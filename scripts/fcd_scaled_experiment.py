#!/usr/bin/env python3
"""F-C+D hybrid loss experiment on 40 cyber behaviors.

Combines F-C's ARCA-style target update with F-D's negative refusal loss
(hybrid CE + lambda * refusal).  The optimizer receives both a positive
signal (match the model's own continuation) and a negative signal (don't
produce refusal tokens).

Usage
-----
    # Full run (~12 h on A100-80 GB):
    python scripts/fcd_scaled_experiment.py --model_path $MODEL_PATH

    # Quick run (5 behaviours, 200 steps, ~1 h):
    python scripts/fcd_scaled_experiment.py --model_path $MODEL_PATH --quick

    # Dry-run (1 behaviour, 5 steps, ~10 min):
    python scripts/fcd_scaled_experiment.py --model_path $MODEL_PATH --dry_run
"""
...
from llm_attacks.minimal_gcg.slot_utils import (
    make_hybrid_ce_refusal_loss_fn,
    make_negative_refusal_loss_fn,
)
...
# BIDs 1-30: same as fc_scaled; BIDs 31-40: new
...
PRIOR_FC_RESULTS: Dict[str, Dict[str, str]] = {
    ...
    "F-C (30 beh scaled)": {
        "conv": "28/30", "prefix": "14/30", "strict": "14/30",
        "content": "10/30", "smooth": "240/252",
    },
}

# ---------------------------------------------------------------------------
# Refusal prefix IDs (built once per model)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Phase 1: SlotGCG + F-C+D hybrid attack
# ---------------------------------------------------------------------------
...
def phase1_attack(
    ...
    for bid in behavior_ids:
        ...
        print(f"\n  BID {bid:>2}  (steps={params['num_steps']}, width={sw}, lam={lam})")
        t0 = time.time()
...
# ---------------------------------------------------------------------------
# OOM recovery helpers (identical to fc_scaled but with improved logic)
# ---------------------------------------------------------------------------
...
def phase4_report(
    ...
    print("\n" + "=" * 70)
    print("  F-C+D HYBRID EXPERIMENT REPORT")
    print("=" * 70)
    ...
    fcd_label = f"F-C+D (lam={lam}, {n_beh} beh)"
    print(f"{fcd_label:<24} {fcd_row['conv']:>8} "
          f"{fcd_row['prefix_asr']:>8} {fcd_row['strict_asr']:>10} "
          f"{fcd_row['content_asr']:>11} {fcd_row['smooth']:>10}")
    ...
    report = {
        ...
        "experiment": f"F-C+D hybrid (lam={lam}, {n_beh} behaviors)",
        "params": params,
        "lam": lam,
...
def main():
    parser = argparse.ArgumentParser(
        description="F-C+D hybrid loss on 40 cyber behaviors")
    ...
    output_dir = Path(cli.output_dir or f"output/fcd_scaled/{ts}")
    output_dir.mkdir(parents=True, exist_ok=True)

    mode = "DRY RUN" if cli.dry_run else ("QUICK" if cli.quick else "FULL")
    print("=" * 70)
    print("  F-C+D Hybrid Experiment (ARCA Target + Refusal Loss)")
    print("=" * 70)
    print(f"  Mode:        {mode}")
    ...
    print(f"  Dataset:     {V2_ALL40_CONFIG}")
    print("=" * 70)
